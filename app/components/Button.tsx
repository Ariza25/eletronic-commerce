  ...
  return (
    <button
      ...
      className={`disabled:opacity-70 disabled:cursor-not-allowed rounded-md hover:opacity-80 transition w-full border-slate-700 flex items-center justify-center gap-2 ${
        outline ? "bg-white" : "bg-slate-700"} ${outline ? "text-slate-700" : "text-white"} ${small ? "py-1 px-2 text-sm font-light border-[1px]" : "py-3 px-4 text-md font-semibold border-2"} 
        ${custom ? custom : ""}
    `}
    >
      {Icon && <Icon size={24} />}
